...
from flask import Flask, render_template, session, redirect, request, current_app
from . import main
...
def get_url():
    return "http://%s/api" % current_app.config['api_host']
...
@main.route('/addapi', methods=['GET', 'POST'])
def addapi():
    headers['authorization'] = session['author']
...
@main.route('/updateapi', methods=['GET', 'POST'])
def updateapi():
    headers['authorization'] = session['author']
    method = request.form.get('method')
    formdata = request.form.get('formdata')  # str
...
@main.route('/updateserverapi', methods=['GET', 'POST'])
def updateserverapi():
    headers['authorization'] = session['author']
    method = request.form.get('method')
    formdata = request.form.get('formdata')  # str
...
@main.route('/zabbixapi', methods=['GET', 'POST'])
def add_zabbix_api():
    headers['authorization'] = session['author']
...
@main.route('/zabbix_template', methods=['GET', 'POST'])
def zabbix_template():
    headers['authorization'] = session['author']
    params = dict(request.form)
